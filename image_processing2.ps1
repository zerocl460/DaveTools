...
# Modified Color-Correct-Files Function to support multiple formats and output to WEBP
Function Color-Correct-Files ($inputFolder, $outputFolder, $isWebpOutput = $false) {
    New-Item -ItemType Directory -Path $outputFolder -Force
    
    Get-ChildItem "$inputFolder\*.*" | Where-Object { $_.Extension -eq '.png' -or $_.Extension -eq '.jpg' } | ForEach-Object {
        $outputFilename = if ($isWebpOutput) {
            Join-Path $outputFolder ($_.BaseName + '.webp')
        } else {
            Join-Path $outputFolder $_.Name
        }

        # Run ImageMagick command to color-correct image with contrast
